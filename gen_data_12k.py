...
# -*- coding: utf-8 -*-
"""
gen_data_12k.py — 生成 AISBench 性能测试用的 GSM8K 混合序列数据集。
...
场景 (本脚本实现的用例):
  ...
  - 混合序列: 10 个 (输入token / 输出token) 长度桶, 按给定占比混合并打散。 本流量模型平均输入/输出≈12k/2k。
  - cache 命中率 25%: 每条 prompt 的前 25% input token 取自同一段全局共享前缀
             (shorter 请求的共享段是 longer 请求共享段的 token 前缀), 其余 75%
             用带唯一标记的、互不相同的 GSM8K 内容填充。开启前缀缓存运行时,
             聚合前缀命中率 ≈ 25%。

运行 (示例):
  python gen_data_12k.py                       # 默认 2500 条, 32 并发, 25% cache
  python gen_data_12k.py --gsm8k-path D:/download/xxx.jsonl --tokenizer /path/to/deepseek-v3.2
  然后:
  ais_bench --models <model> \
            --custom-dataset-path gsm8k_12k_c32_cache25.jsonl \
...
"""gen_data_12k.py 配套的 EvalScope 自定义数据集 plugin(注册 dataset 名 'mixed')。
...
def main():
    ...
    out_path = args.output or ("gsm8k_12k_c%d_cache%d_%s.jsonl" % (
